...
async function loadAllData() {
  try {
    ...
    const requests = [
      ...
      isSubAdmin ? Promise.resolve(null) : AdminAPI.getBridgeDevices(),
      isSubAdmin ? Promise.resolve(null) : AdminAPI.getBridgeEvents(),
    ...
    if (settingsRes?.settings) AdminStore.settings = settingsRes.settings;
...
function applyRoleUI() {
  ...
  if (settingsForm) {
    ...
    settingsForm.querySelectorAll('input, select, textarea, button[type="submit"]').forEach(control => {
      ...
      const isSystemSettingControl = ['platform_name', 'support_email', 'maintenance_mode', 'esim_progress_enabled', 'esim_progress_percent_per_hour'].includes(name);
...
function renderSettingsView() {
  ...
  ['platform_name', 'support_email', 'maintenance_mode', 'esim_progress_enabled', 'esim_progress_percent_per_hour'].forEach(key => {
  ...
  container.innerHTML = AdminStore.packages.map(p => {
    ...
    <div class="dashboard-widget-card" style="padding: 16px;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 12px; padding-top: 10px; border-top: 1px solid var(--border-subtle);">
        ...
        <div style="display:flex; gap:6px;"><button class="btn-action-small view" onclick="openRenewalPricingModal('${p.id}')">Renewal prices</button><button class="btn-action-small view" onclick="handleDeletePackage('${p.id}')">Delete</button></div>
      </div>
    </div>
  `;
  }).join('');
  ...
  container.innerHTML = AdminStore.bridgeDevices.map(d => `
    <div class="dashboard-widget-card" style="padding: 16px;">
      ...
      <div style="font-size: 0.8rem; color: var(--text-gray); margin-bottom: 14px;">
        <div>Binding: <strong style="color: var(--text-white);">${d.provider || 'Unbound'} / ${d.merchant_id || 'No merchant'}</strong></div>
        <div style="margin-top:2px;">Last heartbeat: <strong>${d.last_heartbeat ? new Date(d.last_heartbeat).toLocaleString() : 'Never'}</strong></div>
      ...
      <div style="display: flex; gap: 8px;">
        <button class="btn-action-small view" style="flex: 1;" onclick="provisionBridgeDevice(${d.id})">${d.device_secret || d.credential_hash ? 'Regenerate Secret' : 'Provision'}</button>
...
function renderBridgeEvents() {
  ...
  if (!tbody) return;
  tbody.innerHTML = (AdminStore.bridgeEvents || []).map(event => `<tr><td>${event.bridge_device_id}</td><td>${event.provider}</td><td>${event.transaction_reference}</td><td>UGX ${Number(event.amount || 0).toLocaleString()}</td><td><span class="status-pill ${String(event.status || '').toLowerCase()}">${event.status}</span></td><td>${event.received_at ? new Date(event.received_at).toLocaleString() : '-'}</td></tr>`).join('') || '<tr><td colspan="6" style="text-align:center; padding:24px;">No bridge events received</td></tr>';
}
...
async function registerBridgeDevice(event) {
  ...
  const form = event.currentTarget;
  try {
    await AdminAPI.addBridgeDevice({ device_id: form.device_id.value.trim(), network: form.network.value, phone: form.phone.value.trim(), sim_balance: 0 });
    form.reset();
...
async function provisionBridgeDevice(id) {
  ...
  if (device?.device_secret || device?.credential_hash) {
  ...
  const provider = await showCustomPrompt({ title: 'Provision Bridge', message: 'Provider name (MTN or Airtel):', placeholder: 'MTN', confirmText: 'Continue' });
  if (!provider) return;
  const merchantId = await showCustomPrompt({ title: 'Merchant Binding', message: 'Enter the merchant ID/code this device is allowed to monitor:', placeholder: 'merchant-id', confirmText: 'Provision' });
  if (!merchantId) return;
  try {
    const result = await AdminAPI.provisionBridge(id, { provider, merchant_id: merchantId });
    await loadAllData();
