...
async function fetchBackendDataInternal() {
  ...
  try {
    const supportData = await api.fetchSupportConfig();
    if (supportData?.support) {
      Object.assign(supportContacts, supportData.support);
      withdrawalFee = Number(supportData.support.withdrawalFee) >= 0 ? Number(supportData.support.withdrawalFee) : 2000;
  ...
  try {
    ...
    if (transactionsRes.status === 'fulfilled' && transactionsRes.value?.transactions) {
      renderRecentActivity(transactionsRes.value.transactions.slice(0, 20));
    }
...
function renderSupportContacts() {
  const channels = [
    ...
    {
      key: 'telegram',
      cardId: 'supportTelegramCard',
      valueId: 'supportTelegramValue',
      emptyText: 'Currently unavailable',
      getHref: value => value.startsWith('http') ? value : `https://t.me/${value.replace(/^@/, '')}`
    },
    {
      key: 'email',
      cardId: 'supportEmailCard',
      valueId: 'supportEmailValue',
      emptyText: 'Currently unavailable',
      getHref: value => `mailto:${value}`
    },
    {
      key: 'callCenter',
      cardId: 'supportCallCenterCard',
      valueId: 'supportCallCenterValue',
      emptyText: 'Currently unavailable',
  ...
  channels.forEach(channel => {
    const value = String(supportContacts[channel.key] || '').trim();
    const card = document.getElementById(channel.cardId);
    const valueElement = document.getElementById(channel.valueId);
    if (!card || !valueElement) return;

    valueElement.textContent = value || channel.emptyText;
    card.classList.toggle('support-channel-inactive', !value);
    card.setAttribute('aria-disabled', String(!value));
    if (value) {
      card.href = channel.getHref(value);
      card.removeAttribute('onclick');
    } else {
      card.removeAttribute('href');
      card.onclick = () => showToast('This support channel is currently unavailable', 'info');
    }
  });
}
      ...
      return `
        <div class="activity-card-item">
          <div class="activity-card-left">
            <div class="activity-tag-icon ${isPending ? '' : isIn ? 'in' : 'out'}">
          ...
          <div class="activity-val-text ${isPending ? '' : isIn ? 'green' : ''}">${sign}UGX ${amount.toLocaleString()}</div>
        </div>
      `;
    }).join('');
  });
...
function renderNotificationList(notifications = []) {
  ...
  container.innerHTML = filteredNotifications.map(item => {
    ...
    return `
      <div class="notification-item-card ${isUnread ? 'unread' : ''}" data-notification-id="${item.id || ''}" data-category="${type}">
  ...
  container.querySelectorAll('.notification-item-card').forEach(card => {
    ...
    const notification = filteredNotifications.find(item => String(item.id) === String(notificationId));
...
function openNotificationDetails(notification) {
  ...
  const title = notification.title || 'VSIM Update';
  const message = notification.message || 'No details available for this notification.';
  const timestamp = notification.created_at ? new Date(notification.created_at).toLocaleString([], { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : 'Now';
...
async function confirmMobileMoneyPayment() {
  ...
  try {
    ...
    showToast('Payment submitted. Your eSIM will appear after verification.', 'success');
    appState.targetEsimId = null;
    appState.targetEsimIccid = null;
    await fetchBackendData();
    if (renewedEsimId) await showRenewedEsim(renewedEsimId);
  } catch (error) {
...
function chooseWithdrawNetwork(network, card) {
  selectedWithdrawNetwork = String(network || '').toUpperCase();
  document.querySelectorAll('#withdrawNetworkOptions .network-select-card').forEach(option => option.classList.remove('selected'));
...
async function handleLogin(e) {
  ...
  if (!/^07\d{8}$/.test(phone)) {
    showToast('Enter a valid 10-digit phone number starting with 07', 'error');
    return;
  }
  if (!password) {
...
async function simulateBiometricAuth() {
  ...
  if (!/^07\d{8}$/.test(phone)) {
    showToast('Enter your 10-digit phone number first', 'error');
    phoneInput?.focus();
    return;
  }
  try {
    const options = await window.VSIM_API.getPasskeyLoginOptions(phone);
    const credential = await navigator.credentials.get({ publicKey: {
      ...options,
      challenge: base64UrlToBuffer(options.challenge),
      allowCredentials: (options.allowCredentials || []).map(item => ({ ...item, id: base64UrlToBuffer(item.id) }))
    } });
    const result = await window.VSIM_API.verifyPasskeyLogin(phone, serializeCredential(credential));
    if (result?.user) {
      appState.profile.name = result.user.name || 'User';
      appState.profile.phone = result.user.phone || phone;
      appState.profile.email = result.user.email || '';
      appState.profile.initials = result.user.initials || 'U';
      appState.profile.profilePhoto = result.user.profile_photo || '';
      appState.walletBalance = Number(result.user.wallet_balance) || 0;
      authStatus = 'authenticated';
      localStorage.setItem('vsim_has_account', 'true');
      updateProfileUI();
      updateBalanceDisplay();
    ...
    showToast(error.name === 'NotAllowedError' ? 'Passkey login was cancelled' : (error.message || 'Passkey login failed'), 'error');
  }
}
