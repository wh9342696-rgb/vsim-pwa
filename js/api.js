...
// Public API origin is deployment configuration, never a secret.
...
const VSIM_API = {
  ...
  async getPasskeyLoginOptions(phone) {
    return this.request('/auth/passkey/login/options', { method: 'POST', body: JSON.stringify({ phone }) });
  },

  async verifyPasskeyLogin(phone, response) {
    const data = await this.request('/auth/passkey/login/verify', { method: 'POST', body: JSON.stringify({ phone, response }) });
    if (data.token) this.setToken(data.token);
    return data;
  },
  ...
  async fetchSupportConfig() {
    return await this.request('/support/config');
  },
  ...
  async fetchLiveSupport(ticketId) {
    return await this.request(`/support/live/${ticketId}`);
  },

  async sendLiveSupportMessage(ticketId, body) {
    return await this.request(`/support/live/${ticketId}/messages`, { method: 'POST', body: JSON.stringify({ body }) });
  },
