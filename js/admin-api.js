const AdminAPI = (() => {
  ...
  return {
    ...
    processDeposit: (id, action, reason = '', merchantReference = '', merchantAmount = '') => request(`/deposits/${id}/action`, { method: 'POST', body: JSON.stringify({ action, reason, merchantReference, merchantAmount }) }),
    ...
    processAirtimePurchase: (id, action, merchantReference = '', merchantAmount = '') => request(`/airtime-purchases/${id}/action`, { method: 'POST', body: JSON.stringify({ action, merchantReference, merchantAmount }) }),
    getAirtimeSales: () => request('/airtime-sales'),
    processAirtimeSale: (id, action, merchantReference = '', merchantAmount = '') => request(`/airtime-sales/${id}/action`, { method: 'POST', body: JSON.stringify({ action, merchantReference, merchantAmount }) }),
    ...
    sendTicketMessage: (id, body) => request(`/tickets/${id}/messages`, { method: 'POST', body: JSON.stringify({ body }) }),
    getKyc: () => request('/kyc'),
