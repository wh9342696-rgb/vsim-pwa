...
if (process.env.NODE_ENV === 'production' && !process.env.DB_HOST) {
  throw new Error('DB_HOST must be set in production; SQLite fallback is for development only');
}
...
if (process.env.DB_HOST) {
  ...
  // Local SQLite Engine (Zero configuration required)
  const dbDir = path.join(__dirname, '../database');
  if (!fs.existsSync(dbDir)) {
    fs.mkdirSync(dbDir, { recursive: true });
  }
  const dbPath = path.join(dbDir, 'vsim_local.db');
