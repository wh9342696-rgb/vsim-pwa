...
const PROFILE_PHOTO_PATTERN = /^data:image\/(jpeg|png|webp);base64,[A-Za-z0-9+/]+=*$/;

async function resolveReferralCode(value) {
  const rawValue = String(value || '').trim();
  if (!rawValue) return null;
  const tokenResult = await query(
    'SELECT u.id, u.name, u.phone, u.referral_code FROM referral_tokens rt JOIN users u ON u.id = rt.user_id WHERE rt.token = $1',
    [rawValue]
  );
...
const signupSchema = z.preprocess((value) => {
  ...
  phone: z.string().regex(/^07\d{8}$/, 'Enter a valid 10-digit phone number starting with 07'),
...
router.post(['/signup', '/register'], async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      ...
      JWT_SECRET,
      { expiresIn: '7d', algorithm: 'HS256', issuer: 'vsim-api', audience: 'vsim-client' }
    );
...
router.post('/login', async (req, res) => {
  try {
    ...
    const { password_hash, referral_code, referred_by, ...safeUser } = user;
...
router.post('/passkey/register/options', authenticateToken, async (req, res) => {
  try {
    ...
    const options = await generateRegistrationOptions({
      ...
      rpID: RP_ID,
      userID: Uint8Array.from(Buffer.from(String(user.id), 'utf8')),
      userName: user.phone,
...
router.post('/passkey/login/options', async (req, res) => {
  try {
    ...
    if (!/^07\d{8}$/.test(phone)) return res.status(400).json({ error: 'Enter a valid 10-digit phone number' });
    ...
    await saveChallenge(user.id, phone, options.challenge, 'login');
...
router.post('/passkey/login/verify', async (req, res) => {
  try {
    const phone = String(req.body?.phone || '').replace(/\s+/g, '').trim();
    if (!/^07\d{8}$/.test(phone) || !req.body?.response) return res.status(400).json({ error: 'Passkey login details are required' });
    const user = (await query('SELECT * FROM users WHERE phone = $1', [phone])).rows[0];
    const passkey = user ? await getPasskey(user.id) : null;
    const challenge = user ? await consumeChallenge(user.id, phone, 'login') : null;
    if (!user || !passkey || !challenge) return res.status(400).json({ error: 'Passkey login expired. Try again.' });
    ...
    const sessionToken = createSessionToken();
    await query('UPDATE users SET current_session_token = $1 WHERE id = $2', [sessionToken, user.id]);
    const token = jwt.sign({ id: user.id, phone: user.phone, name: user.name, sessionToken }, JWT_SECRET, { expiresIn: '7d', algorithm: 'HS256', issuer: 'vsim-api', audience: 'vsim-client' });
    const { password_hash, ...safeUser } = user;
    res.json({ message: 'Logged in with passkey', token, user: safeUser });
  } catch (err) {
    console.error('Passkey login verification error:', err);
    res.status(400).json({ error: 'Passkey login could not be completed' });
  }
});
...
router.put('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    if (profilePhoto && (profilePhoto.length > MAX_PROFILE_PHOTO_LENGTH || !PROFILE_PHOTO_PATTERN.test(profilePhoto))) {
      return res.status(400).json({ error: 'Profile photo must be a JPG, PNG or WebP image up to 2 MB' });
    }
