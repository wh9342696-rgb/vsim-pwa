...
router.post('/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: admin.id, email: admin.email, role: admin.role, sessionToken }, JWT_SECRET, { expiresIn: '7d', algorithm: 'HS256', issuer: 'vsim-api', audience: 'vsim-admin' });
...
router.get('/settings', adminAuth, async (req, res) => {
  ...
  res.json({ canManageWithdrawalFee: req.admin.role === 'super_admin' || Boolean(req.admin.can_manage_withdrawal_fee), settings: {
    ...
    airtime_sell_payout_percent: settings.airtime_sell_payout_percent ?? '90',
    ...settings
  } });
});
...
router.post('/deposits/simulate', adminAuth, async (req, res) => {
  try {
    ...
    const ref = await createUniqueReference('SIM-DEP', async candidate => (await query('SELECT id FROM payment_requests WHERE reference = $1', [candidate])).rows.length > 0);
...
router.post('/packages', adminAuth, ensureSuperAdmin, async (req, res) => {
  try {
    const { id, country, title, validity, data_quota, price, income, commission_percent, image_url, region, progress_percent_per_hour, renewal_schedule } = req.body;
    const pkgId = id || `pkg_${Date.now()}`;
    const schedule = normalizeRenewalSchedule(renewal_schedule, price);
    await query(
      `INSERT INTO esim_packages (id, country, title, validity, data_quota, price, income, commission_percent, image_url, region, progress_percent_per_hour, renewal_schedule, sold_count, revenue)
      ...
      [
        ...
        Math.max(0, Math.min(100, parseFloat(commission_percent) || 10)),
        image_url || '',
        region || 'global',
...
router.put('/packages/:id', adminAuth, ensureSuperAdmin, async (req, res) => {
  try {
    const { title, validity, data_quota, price, income, commission_percent, status, progress_percent_per_hour, renewal_schedule } = req.body;
    ...
    await query(
      `UPDATE esim_packages 
       SET title = COALESCE($1, title),
        ...
        WHERE id = $9`,
          [title, validity, data_quota, price ? parseFloat(price) : null, income ? parseFloat(income) : null, progress_percent_per_hour !== undefined ? Math.max(0, Math.min(100, parseFloat(progress_percent_per_hour) || 0)) : null, schedule, commission_percent !== undefined ? Math.max(0, Math.min(100, parseFloat(commission_percent) || 0)) : null, id]
    );
...
router.get('/bridge-devices', adminAuth, async (req, res) => {
  try {
...
router.post('/bridge-devices/:id/provision', adminAuth, ensureSuperAdmin, async (req, res) => {
  try {
    ...
    const { provider, merchant_id, mtn_merchant_id, airtel_merchant_id, app_version } = req.body || {};
    const result = await query(
      `UPDATE bridge_devices SET status = 'provisioning', provider = $1, merchant_id = $2, mtn_merchant_id = $3, airtel_merchant_id = $4, app_version = $5, credential_hash = $6, device_secret = $7, revoked_at = NULL WHERE id = $8 RETURNING device_id, status, provider, merchant_id, mtn_merchant_id, airtel_merchant_id, app_version`,
      [provider || null, merchant_id || null, mtn_merchant_id || null, airtel_merchant_id || null, app_version || null, credentialHash, encryptBridgeSecret(deviceSecret), req.params.id]
    );
...
router.post('/bridge-devices', adminAuth, async (req, res) => {
  try {
    ...
    if (!normalizedDeviceId) return res.status(400).json({ error: 'Device ID is required' });
    const existing = await query('SELECT id, status FROM bridge_devices WHERE device_id = $1', [normalizedDeviceId]);
    if (existing.rows[0]) {
      return res.status(409).json({ error: `Device ${normalizedDeviceId} is already registered (${existing.rows[0].status}). Use the existing device card to provision or update it.` });
    }
    const deviceSecret = crypto.randomBytes(16).toString('hex');
    const credentialHash = hashBridgeSecret(deviceSecret);
    await query(
...
router.get('/tickets', adminAuth, async (req, res) => {
  try {
    const result = await query(`SELECT st.*, u.name AS user_name, u.phone AS user_phone, a.name AS assigned_admin_name FROM support_tickets st LEFT JOIN users u ON u.id = st.user_id LEFT JOIN admin_users a ON a.id = st.assigned_admin_id ${req.admin.role === 'sub_admin' ? 'WHERE st.assigned_admin_id = $1' : ''} ORDER BY st.created_at DESC LIMIT 100`, req.admin.role === 'sub_admin' ? [req.admin.id] : []);
...
router.patch('/tickets/:id', adminAuth, async (req, res) => {
  try {
    ...
    if (assignedAdminId !== undefined && req.admin.role !== 'super_admin') {
      return res.status(403).json({ error: 'Only the main admin can assign support chats' });
    }
    if (assignedAdminId !== undefined && assignedAdminId !== null && assignedAdminId !== '') {
      const assignee = await query("SELECT id FROM admin_users WHERE id = $1 AND role = 'sub_admin' AND status = 'active'", [assignedAdminId]);
      if (!assignee.rows[0]) return res.status(400).json({ error: 'Choose an active sub-admin' });
    }
    if (req.admin.role === 'sub_admin') {
      const owned = await query('SELECT id FROM support_tickets WHERE id = $1 AND assigned_admin_id = $2', [req.params.id, req.admin.id]);
      if (!owned.rows[0]) return res.status(403).json({ error: 'This support chat is not assigned to you' });
    }
    const result = await query(`UPDATE support_tickets SET status = COALESCE($1, status), priority = COALESCE($2, priority), assigned_admin_id = COALESCE($3, assigned_admin_id), assigned_at = CASE WHEN $3 IS NOT NULL THEN CURRENT_TIMESTAMP ELSE assigned_at END WHERE id = $4 RETURNING *`, [status, priority, assignedAdminId ?? null, req.params.id]);
    if (!result.rows[0]) return res.status(404).json({ error: 'Support ticket not found' });
    if (result.rows[0]) emitDataChanged('support', { type: 'support', ticketId: result.rows[0].id, audience: [{ type: 'admin_role', role: 'super_admin' }, ...(result.rows[0].assigned_admin_id ? [{ type: 'admin', id: result.rows[0].assigned_admin_id }] : []), ...(result.rows[0].user_id ? [{ type: 'user', id: result.rows[0].user_id }] : [])] });
...
router.get('/tickets/:id/messages', adminAuth, async (req, res) => {
  try {
    const access = req.admin.role === 'sub_admin' ? 'AND assigned_admin_id = $2' : '';
    const params = req.admin.role === 'sub_admin' ? [req.params.id, req.admin.id] : [req.params.id];
    const ticket = await query(`SELECT * FROM support_tickets WHERE id = $1 ${access}`, params);
    if (!ticket.rows[0]) return res.status(404).json({ error: 'Support ticket not found' });
    const messages = await query('SELECT * FROM support_messages WHERE ticket_id = $1 ORDER BY created_at ASC, id ASC', [req.params.id]);
    res.json({ ticket: ticket.rows[0], messages: messages.rows });
  } catch (err) { res.status(500).json({ error: 'Failed to fetch support messages' }); }
});

router.post('/tickets/:id/messages', adminAuth, async (req, res) => {
  try {
    const body = String(req.body?.body || '').trim();
    if (!body || body.length > MAX_SUPPORT_MESSAGE_LENGTH) return res.status(400).json({ error: 'Message must be between 1 and 2,000 characters' });
    const access = req.admin.role === 'sub_admin' ? 'AND assigned_admin_id = $2' : '';
    const params = req.admin.role === 'sub_admin' ? [req.params.id, req.admin.id] : [req.params.id];
    const ticket = await query(`SELECT id FROM support_tickets WHERE id = $1 AND status NOT IN ('closed', 'resolved') ${access}`, params);
    if (!ticket.rows[0]) return res.status(404).json({ error: 'Support ticket is unavailable' });
    const result = await query(`INSERT INTO support_messages (ticket_id, sender_type, sender_id, body) VALUES ($1, 'admin', $2, $3) RETURNING *`, [req.params.id, req.admin.id, body]);
    await query("UPDATE support_tickets SET status = 'in_progress' WHERE id = $1", [req.params.id]);
    const owner = await query('SELECT user_id, assigned_admin_id FROM support_tickets WHERE id = $1', [req.params.id]);
    emitDataChanged('support', { type: 'support', ticketId: req.params.id, audience: [{ type: 'admin_role', role: 'super_admin' }, { type: 'admin', id: req.admin.id }, ...(owner.rows[0]?.user_id ? [{ type: 'user', id: owner.rows[0].user_id }] : [])] });
    res.status(201).json({ message: result.rows[0] });
  } catch (err) { res.status(500).json({ error: 'Failed to send support message' }); }
});
