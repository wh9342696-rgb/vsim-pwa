...
- Keep port 3000 bound to loopback; expose HTTPS through Nginx on port 443.
- Set `WEBAUTHN_RP_ID` and `WEBAUTHN_ORIGIN` to the production HTTPS domain.
- Run `powershell -ExecutionPolicy Bypass -File .\smoke-test-production.ps1 -BaseUrl https://your-domain.com` after deployment.
